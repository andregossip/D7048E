...
FingerCount::FingerCount(void) {
	...
	currentDirection = NULL; // 0: LEFT, 1: TOP, 2: RIGHT, 3:BOTTOM
}

void doInput(CHAR character, int& currentDirection) {
	if (currentDirection != character) {
		...
		ip.ki.wVk = currentDirection; // virtual-key code for the "a" key
...
Mat FingerCount::findFingersCount(Mat input_image, Mat frame) {
	...
	//right 
	if (center_bounding_rect.x > 150 && (center_bounding_rect.y > 100
		&& center_bounding_rect.y < 300)) {
		circle(contours_image, center_bounding_rect, 5, color_green, 2, 8);
		doInput(0x27, currentDirection);
		std::cout << "right" << std::endl;
	}
	//left 
	else if (center_bounding_rect.x < 150 && (center_bounding_rect.y > 100 && center_bounding_rect.y < 300)) {
		circle(contours_image, center_bounding_rect, 5, color_purple, 2, 8);
		doInput(0x25, currentDirection);
		std::cout << "left" << std::endl; 
	}
	//up
	else if (center_bounding_rect.y < 100) {
		circle(contours_image, center_bounding_rect, 5, color_red, 2, 8);
		doInput(0x26, currentDirection);
		std::cout << "up" << std::endl;
	}
	//down
	else if (center_bounding_rect.y > 300) {
		circle(contours_image, center_bounding_rect, 5, color_blue, 2, 8);
		doInput(0x28, currentDirection);
